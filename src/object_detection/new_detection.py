...
1 dedicated thread for fetching camera input

1 thread for detection results and displaying the video

Hopefully both threads working in a pipeline fashion will improve
FPS of video output. 
...
WINDOW_NAME = 'Sports Ball Tracker'
...
DEFAULT_MODEL = SUPPORTED_MODELS[2]
...
class TrtThread(threading.Thread):
    """
    Thread to read image / do detection and store them in a global
    variables so the main thread can read and display output
    """
...
def main():
    ...
    vis = BBoxVisualization(DEFAULT_MODEL)
